...
function VMDetails({ id }) {
  ...
  if (vms.items !== undefined && vms.items.length > 0) {
    ...
    let caps
    if (
      vm.setup.providedCapabilities === undefined ||
      vm.setup.providedCapabilities.length === 0
    ) {
      caps = <>&ndash;</>
    } else {
      caps = vm.setup.providedCapabilities.map((r, i) => (
        <>
          <Label key={i}>{r}</Label>
          <wbr />
        </>
    ...
    result = (
      <>
            ...
            <DefinitionList>
              ...
              <DefinitionListItem title="Uptime">
                {vm.creationTime && vm.destructionTime ? (
                  ...
                  <LiveDuration startTime={vm.creationTime} />
                ) : (
                  <>&ndash;</>
                )}
              </DefinitionListItem>
          ...
          <div className="vm-details-right">
            <DefinitionList>
              <DefinitionListItem title="Flavor">
                <span className="vm-details-value">
        ...
        <div className="vm-details-two-column">
          <div className="vm-details-left">
            <DefinitionList>
              <DefinitionListItem title="ID">{vm.setup.id}</DefinitionListItem>
              <DefinitionListItem title="Block device size">
                {(vm.setup.blockDeviceSizeGb &&
                  `${vm.setup.blockDeviceSizeGb} GB`) || <>&ndash;</>}
              </DefinitionListItem>
          ...
          <div className="vm-details-right">
            <DefinitionList>
              <DefinitionListItem title="Minimum">
                {(vm.setup.minVMs && `${vm.setup.minVMs} instances`) || (
                  <>&ndash;</>
                )}
              </DefinitionListItem>
              <DefinitionListItem title="Maximum">
                {(vm.setup.maxVMs && `${vm.setup.maxVMs} instances`) || (
                  <>&ndash;</>
                )}
              </DefinitionListItem>
              <DefinitionListItem title="Create concurrently">
                {(vm.setup.maxCreateConcurrent &&
                  `max. ${vm.setup.maxCreateConcurrent} instances`) || (
                  <>&ndash;</>
                )}
              </DefinitionListItem>
            </DefinitionList>
          ...
          <div className="vm-details-left">
            <DefinitionList>
              <DefinitionListItem title="Additional volumes">
                {(vm.setup.additionalVolumes === undefined ||
                  vm.setup.additionalVolumes.length === 0) && <>&ndash;</>}
                {vm.setup.additionalVolumes !== undefined &&
                  vm.setup.additionalVolumes.length > 0 && (
                    <div className="vm-details-volume-grid">
                      {vm.setup.additionalVolumes.map(volume => (
                        <>
                          <div className="vm-details-volume-size">
                            {volume.sizeGb} GB
                          </div>
                          <div className="vm-details-volume-type">
                            {volume.type || "(auto)"}
                          </div>
                          <div className="vm-details-volume-availability-zone">
            ...
            <DefinitionList>
              <DefinitionListItem title="Maximum number of attempts">
                {vm.setup.creation?.retries?.maxAttempts !== undefined ? (
                  vm.setup.creation.retries.maxAttempts
                ) : (
                  <>(default)</>
                )}
              </DefinitionListItem>
              <DefinitionListItem title="Delay between attempts">
                {vm.setup.creation?.retries?.delay !== undefined ? (
                  formatDurationMilliseconds(
                    vm.setup.creation.retries.delay,
                    true
                  )
                ) : (
                  <>(default)</>
          ...
          <div className="vm-details-left">
            <DefinitionList>
              <DefinitionListItem title="Exponential backoff factor">
                {vm.setup.creation?.retries?.exponentialBackoff !==
                undefined ? (
                  vm.setup.creation.retries.exponentialBackoff
                ) : (
                  <>(default)</>
              ...
              <DefinitionListItem title="Maximum delay between attempts">
                {vm.setup.creation?.retries?.maxDelay !== undefined ? (
                  formatDurationMilliseconds(
                    vm.setup.creation.retries.maxDelay,
                    true
                  )
                ) : (
                  <>(default)</>
            ...
            <DefinitionList>
              <DefinitionListItem title="Lock time after all retries">
                {vm.setup.creation?.lockAfterRetries !== undefined ? (
                  formatDurationMilliseconds(
                    vm.setup.creation.lockAfterRetries,
                    true
                  )
                ) : (
                  <>(default)</>
