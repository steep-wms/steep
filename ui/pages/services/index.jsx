...
const Services = () => {
  const { data: services, error } = useSWRImmutable(
    `${process.env.baseUrl}/services`,
    fetcher
  )
  ...
  return (
    <Page title="Services">
      <h1>Services</h1>
      {serviceElements}
      {serviceElements && serviceElements.length === 0 && (
        <>There are no services.</>
      )}
      {error && <Alert error>Could not load services</Alert>}
      {services && services.length === 0 && (
        <Alert warning>There are no configured services</Alert>
      )}
    </Page>
  )
}
