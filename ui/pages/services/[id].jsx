...
const Service = () => {
  ...
  const { data, error } = useSWRImmutable(
    () => id && `${process.env.baseUrl}/services/${id}`,
    fetcher
  )
  ...
  if (data !== undefined) {
    ...
    if (
      ...
      reqcap = data.requiredCapabilities.map((r, i) => (
        <>
          <Label key={i}>{r}</Label>
          <wbr />
        </>
          ...
          <DefinitionList>
            <div className="service-details-dl">
              ...
              <div className="service-details-right">
                <DefinitionListItem title="Runtime">
                  <Label>{data.runtime}</Label>
                </DefinitionListItem>
                {data.runtime !== "docker" ? (
          ...
          <>
            ...
            <div className="retry-policy-details">
              <DefinitionList>
                <div className="service-details-dl">
                  <div className="service-details-left">
                    <DefinitionListItem title="Max. attempts">
                      {data.retries.maxAttempts || 1}
                    </DefinitionListItem>
                    <DefinitionListItem title="Delay">
                      {(data.retries.delay &&
                        formatDurationMilliseconds(data.retries.delay, true)) ||
                        "0ms"}
                    </DefinitionListItem>
                  </div>
                  <div className="service-details-right">
                    <DefinitionListItem title="Exponential backoff factor">
                      {data.retries.exponentialBackoff || 1}
                    </DefinitionListItem>
                    <DefinitionListItem title="Maximum delay">
                      {(data.retries.maxDelay &&
                ...
                <div className="service-details-dl">
                  <div className="service-details-left">
                    <DefinitionListItem title="Max. inactivity">
                      {(data.maxInactivity && (
                        <>
                          {data.maxInactivity.errorOnTimeout
                            ? "Fail"
                            : "Cancel"}{" "}
                          after{" "}
                          {formatDurationMilliseconds(
                            data.maxInactivity.timeout,
                            true
                          )}
                        </>
                      )) || <>&ndash;</>}
                    ...
                    <DefinitionListItem title="Max. runtime">
                      {(data.maxRuntime && (
                        <>
                          {data.maxRuntime.errorOnTimeout ? "Fail" : "Cancel"}{" "}
                          after{" "}
                          {formatDurationMilliseconds(
                            data.maxRuntime.timeout,
                            true
                          )}
                        </>
                      )) || <>&ndash;</>}
                  ...
                  <div className="service-details-right">
                    <DefinitionListItem title="Deadline">
                      {(data.deadline && (
                        <>
                          {data.deadline.errorOnTimeout ? "Fail" : "Cancel"}{" "}
                          after{" "}
                          {formatDurationMilliseconds(
                            data.deadline.timeout,
                            true
                          )}
                        </>
                      )) || <>&ndash;</>}
            ...
            {data.runtimeArgs.map(r => (
              <div className="service-parameter" key={r.id}>
                ...
                <div className="service-parameter-right">
                  ...
                  <h5>Label:</h5>
                  <span className="service-parameter-value">
                    {r.label || none}
                  </span>
            ...
            {data.parameters.map(p => (
              <div className="service-parameter" key={p.id}>
                ...
                <div className="service-parameter-right">
                  ...
                  <h5>Default value:</h5>
                  <span className="service-parameter-value">
                    {p.default === undefined ? none : "" + p.default}
                  </span>
