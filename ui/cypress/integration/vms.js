  ...
  describe(`${href} VMs item page`, () => {
    it("contains complete header", () => {
      ...
      cy.get(".detail-page-title > h1")
        .should("be.visible")
        .should("include.text", href.split("/")[0])
      cy.get(".breadcrumbs > :nth-child(1) > a")
        .should("be.visible")
        .should("include.text", "VMs")
      cy.get(".breadcrumbs > :nth-child(1) > a")
        .invoke("attr", "href")
        .should("eq", "/vms/")
    ...
    it("contains complete detail header", () => {
      ...
      cy.get(".detail-header > .detail-header-left > .definition-list")
        .children()
        .should("be.visible")
        .should("have.length", 8)
        .invoke("text")
      let detail_page_main_types = [
        "Creation time",
        "Time when the agent joined the cluster",
        "Destruction time",
        "Uptime"
      ]
    ...
    it("contains an error message", () => {
      cy.get(".detail-page-main > h2")
        .eq(0)
        .should("be.visible")
        .should("include.text", "Error message")
    ...
    it("contains complete VM setup", () => {
      ...
      cy.get(".vm-details-two-column")
        ...
        .should("be.visible")
        .invoke("text")
      let detail_vms_types = [
        "ID",
        "Block device size",
        "Block device volume type",
        "Additional volumes",
        "Minimum",
        "Maximum",
        "Create concurrently"
