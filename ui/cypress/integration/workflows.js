...
describe("Workflow page cancelling", () => {
  ...
  it("Cancels an exisiting workflow", { retries: { runMode: 3, openMode: 0 } }, () => {
    ...
    cy.visit("/workflows")
    cy.get(".list-page").contains(res.body.id).parentsUntil(".list-page")
      .find(".list-item-right > .list-item-progress-box > div > strong").then(($el) => {
...
describe("has table", () => {
  ...
  it("shows all items", () => {
    for (let i = 2; i <= 11; i++) {
      cy.get(`.list-page > :nth-child(${i}) > .list-item-left > .list-item-title > a`,
        { timeout: 10000 }).should("have.text", res[i - 2].body.id)
    }
    cy.get(".pagination > :last").click()
    for (let i = 2; i <= 11; i++) {
      cy.get(`.list-page > :nth-child(${i}) > .list-item-left > .list-item-title > a`,
        { timeout: 10000 }).should("have.text", res[i - 2 + 10].body.id)
    }
  })
  ...
  it("has time dialog box", () => {
    for (let i = 2; i <= 11; i++) {
      ...
      cy.get(`.list-page > :nth-child(${i}) > .list-item-left > .list-item-subtitle > .tooltip`,
        { timeout: 10000 }).should("be.not.visible")
      cy.get(`.list-page > :nth-child(${i}) > .list-item-left > .list-item-subtitle > span > time`,
        { timeout: 10000 }).trigger("mouseover")
      cy.get(`.list-page > :nth-child(${i}) > .list-item-left > .list-item-subtitle > .tooltip`,
